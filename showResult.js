...
function showBsModel(msg, list, showType, customType) {
    ...
    } else if (showType == "notify") {
        modalTitle.text("Error Occurred");
        var gotBtn = '<button type="button" class="btn btn-primary" id="momane_got">Got it</button>';
        modalFooter.html(gotBtn);
        $("#momane_got").click(function(){
            sm({cmd: "removeFrame"});
        });
        modalMsgBody.html("<h5>" + msg.title + "</h5>");
        modalMsgBody.append("<h6 style='color:red;font-size: 13px;font-weight: bolder;'>" + msg.message + "</h6>");
